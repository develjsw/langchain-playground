...
텍스트를 벡터로 변환하는 방법 정리, LangChain은 여러 providers를 동일 인터페이스로 제공
...
모든 임베딩 providers는 두 메서드로 통일됨 → providers만 갈아끼우면 됨
...
- 일부 providers는 문서용/쿼리용 처리를 다르게 함 → 그래서 메서드가 둘로 나뉨
...
- 전체 providers 목록(40+): https://docs.langchain.com/oss/python/integrations/embeddings
- OpenAI 상세 → `openai/`
