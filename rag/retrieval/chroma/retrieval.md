...
`Chroma` 벡터 스토어를 retriever로 변환해 검색하는 방법 정리
> 검색 전략 전체 개요는 상위 `../retrieval.md` 참고
> 이전 단계(벡터 스토어) 전제는 `../../vector-store/chroma/vector-store.md` 참고

## 1. 전제 (구성)

- 설치: `pip install -qU "langchain-chroma>=0.1.2" langchain-openai`
- retriever는 벡터 스토어가 있어야 만들 수 있음 → 벡터 스토어 단계에서 만든 `vector_store` 사용
- 이미 영속화한 DB가 있으면 재임베딩 없이 `Chroma(persist_directory=...)`로 불러와 변환 가능
