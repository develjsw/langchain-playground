...
`UnstructuredLoader`의 청킹 옵션 정리
> 전체 청킹 선택지 개요는 상위 `../chunking.md` 참고
> 로딩(파싱)·설치 등 전제 내용은 `../../loading/loader.md` 참고

## 1. 전제 (구성)

- `langchain-unstructured`(어댑터) + `unstructured`(엔진) 조합으로 처리 (상세 → `../../loading/loader.md`)
...
- 오픈소스는 위 2개뿐 (`by_page`·`by_similarity`는 상용 전용 → 4장)
...
- `by_page`(페이지 경계)·`by_similarity`(임베딩 유사도)는 **상용 Platform/API 전용** (무료 15,000페이지 + 초과분 과금), 오픈소스 라이브러리엔 없음
- 출처: [오픈소스 chunking](https://docs.unstructured.io/open-source/core-functionality/chunking) · [Platform chunking](https://docs.unstructured.io/platform/chunking)

## 5. `RecursiveCharacterTextSplitter`와 함께 쓰려면

- `UnstructuredLoader`는 `mode` 미지원, 기본은 **요소(element)당 Document**
- 전체를 한 덩이로 받으려면 `chunking_strategy="basic"` + `max_characters`=거대값 + `include_orig_elements=False` **우회 필요** → 부자연스러워 권장 X
- 재귀 분할이 목적이면 docling(`ExportType.MARKDOWN`)이나 docx2txt 직접 호출이 더 적합 → 상위 `../chunking.md` 참고
