# Unstructured 로딩(파싱) 정리

`UnstructuredLoader`로 문서를 파싱하는 내용 정리
> 전체 로더 선택지 개요는 상위 `../loader.md` 참고
> 청킹은 `../../chunking/unstructured/chunking.md` 참고
...
- `load_and_split()` 은 **Deprecated** → 사용하지 말 것 (표준은 `load()` → splitter의 `split_documents()`)
- 청킹은 `unstructured`가 옵션으로 지원하므로, 별도 splitter 없이 로더 옵션으로 처리 (→ `../../chunking/unstructured/chunking.md`)
...
- `unstructured` 오픈소스 문서: https://docs.unstructured.io/open-source/core-functionality/overview
